...
namespace model
{
	...
	Dog::Dog(Coordinates coords, const Map *map)
		: position_(coords), speed_(map->GetDogSpeed()), current_map_(map)
	{
	...
	void Dog::MoveVertical(double distance, int iteration)
	{
		...
		double y1 = small_y - OFFSET;
		double y2 = big_y + OFFSET;
	...
	std::string Players::MakePlayer(const std::string &username, const Map *map)
	{
		Player player{CreateDog(map), players_.size(), username, map, *this};
	...
	Dog *Players::CreateDog(const Map *map)
	{
	...
	void Player::Depot()
	{
		score_ = std::accumulate(bag_.begin(), bag_.end(), score_, [](int total, const Item &item)
