...
namespace model
{
	...
	public:
		...
		std::string MakePlayer(const std::string& username, const Map* map);

		Player* FindPlayerByIdx(size_t idx);
		Player* FindPlayerByToken(std::string token) const;

		Dog* CreateDog(const Map* map);
		...
		void InsertPlayer(Player& player, const std::string& token, const std::string& map_id);
	...
	public:
		...
		std::string SpawnPlayer(const std::string& username, const Map* map)
		{
